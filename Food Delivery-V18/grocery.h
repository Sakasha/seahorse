...
void groceryToLowerCase(char *str)
{
    for (int i = 0; str[i]; i++)
    {
        str[i] = tolower(str[i]);
...
void groceryDisplayCart(GroceryCartItem cart[], int cartCount)
{
    printf("Your cart:\n");
    printf("-------------------------------------------------------------------------------\n");
...
void generateBill(GroceryCartItem cart[], int cartCount, Store *store)
{
    float totalCost = 0;
    for (int i = 0; i < cartCount; i++)
    {
...
void setStoreDistances(Store stores[], int storeCount)
{
    for (int i = 0; i < storeCount; i++)
    {
        if (strcmp(stores[i].name, "The Green Grocer") == 0)
            stores[i].distance = 3.0;
        else if (strcmp(stores[i].name, "Garden Fresh Foods") == 0)
            stores[i].distance = 4.0;
        else if (strcmp(stores[i].name, "Harvest Haven Grocery") == 0)
            stores[i].distance = 6.0;
        else if (strcmp(stores[i].name, "Nature's Pantry") == 0)
            stores[i].distance = 7.0;
        else if (strcmp(stores[i].name, "Farm to Fork Market") == 0)
            stores[i].distance = 8.0;
        else if (strcmp(stores[i].name, "The Organic Oasis") == 0)
            stores[i].distance = 9.0;
        else if (strcmp(stores[i].name, "Market Fresh Provisions") == 0)
            stores[i].distance = 10.0;
        else if (strcmp(stores[i].name, "Urban Harvest Grocery") == 0)
            stores[i].distance = 11.0;
        else if (strcmp(stores[i].name, "Healthy Harvest Marketplace") == 0)
            stores[i].distance = 6.0;
        else if (strcmp(stores[i].name, "Pure Produce Market") == 0)
            stores[i].distance = 7.0;
    }
}

int groceryMain(char *userLocation)
{
    Store stores[MAX_STORES];
    int storeCount;

    readFile("./data/grocery.txt", stores, &storeCount);

    // Set distances for stores
    setStoreDistances(stores, storeCount);

    printf("\n Uer Location: %s\n", userLocation);
    groceryToLowerCase(userLocation);
    ...
    GroceryCartItem cart[MAX_ITEMS];
    int cartCount = 0;
    char addItem = 'y';
    ...
    groceryDisplayCart(cart, cartCount);
    ...
    writeFile("./data/grocery.txt", stores, storeCount); // Save the updated stock to the file
